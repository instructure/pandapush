...
var program      = require('commander'),
    ...
    _            = require('lodash'),
    applications = require('../app/lib/applications');
...
program
  ...
  .action(function(applicationId, keyId, channel, options) {
    applications.getApplications(function(err, apps) {
      var app = apps[applicationId];
      if (!app) {
        console.log("could not find application");
        return;
      }

      var key = app.keys[keyId];
      if (!key) {
        console.log("could not find key");
        return;
      }

      var expires = options.expires || 60;
      ...
      var token = jwt.sign(payload, key.secret, { expiresInMinutes: expires });
